...
void SelectGate::updatePosition(){
	...
	for (int i = 0; i < _items.size(); i++){
		...
		//设置伸缩比例
		float scale = menuSize.width / _items.at(i)->getContentSize().width / 2;
		//_items.at(i)->setScale(1.0 - abs(calcFunction(i - _index, MENU_SCALE)));
		_items.at(i)->setScale(scale);
...
void SelectGate::updatePositionWithAnimation(){
	...
	auto menuSize = getContentSize();
	for (int i = 0; i < _items.size(); i++){
		_items.at(i)->setZOrder(-abs((i - _index) * 100));//根据当前中心图片计算图片位置
		float x = calcFunction(i - _index, menuSize.width / 2);
		//从正中间移动到初始位置
		...
		float scale = menuSize.width / _items.at(i)->getContentSize().width / 2;
		//调整到规定大小
		auto scaleTo = ScaleTo::create(ANIMATION_DURATION, scale);
...
void SelectGate::onTouchEnded(Touch* touch, Event* event){
	...
	rectify(xDelta>0);
	if (abs(xDelta)<size.width / 24 && _selectedItem)//如果移动范围小，视为选中
...
void SelectGate::onTouchMoved(Touch* touch, Event* event){
	...
	auto size = getContentSize();
	_lastIndex = _index;
	_index -= xDelta / (size.width *ITEM_SIZE_SCALE);//滑动距离与item显示同步
	updatePosition();
	return;
}
//index边界情况
